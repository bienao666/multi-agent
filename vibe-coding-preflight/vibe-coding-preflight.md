...
12. 在准备工作需要时自动查询合适的插件、skill、MCP tool 或项目本地工具辅助。
13. 完成准备后，再询问用户是否进入编码阶段。
...
执行 Preflight 时，如果任务需要专业能力，你必须自动查询合适的插件、skill、MCP tool 或项目本地工具辅助。

触发场景：

- 需要根据现有代码、README、配置文件判断技术栈或测试方式。
- 需要处理图片、PDF、表格、文档、设计稿或浏览器页面。
- 需要生成结构化文档、图表、架构图、项目计划或检查清单。
- 需要做隐私脱敏、安全判断、依赖风险判断或复杂技术选型。
- 用户要求“查一下”“验证一下”“生成文档”“看页面”“根据附件整理”等。

查询和调用规则：

- 如果当前环境提供工具发现能力，优先查询相关插件或 skill。
- 如果没有工具发现能力，根据当前可见工具和项目文件保守判断。
- 只查询与当前 Preflight 步骤直接相关的能力，不全量读取插件或 skill 文档。
- 调用前说明使用什么能力、为什么适合、不可用时怎么降级。
- 没有合适能力时，继续使用本地文档化流程，不阻塞 Preflight。
- 不要为了使用插件或 skill 而提前写业务代码。
...
1. **Project Scan**
   ...
   - 如果需要识别技术栈、设计稿、附件或复杂文档，按需查询合适插件或 skill。
