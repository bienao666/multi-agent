...
这个目录可以借用 Ponytail 或类似“最小实现 / 输出压缩 / YAGNI”能力，但用途不是写代码，而是控制上下文和产出膨胀。

AI 应先判断：

- 是否真的需要新增人物、项目、事件或模板。
- 是否可以复用或更新现有 `.work-context/` 文件。
- 是否可以用摘要和来源路径代替复制完整材料。
- 输出是否已经覆盖结论、事实、影响、风险、下一步和可能追问。

如果环境没有 Ponytail 或类似能力，则使用 `work-context-system.md` 中的本地“最小充分上下文原则”。
...
处理工作材料时，如果输入包含图片、PDF、Word、表格、PPT、网页、聊天记录截图，或需要生成正式文档、表格、演示稿、图表、流程图、隐私脱敏结果，AI 会自动查询合适的插件、skill、MCP tool 或本地工具。如果存在 Ponytail、最小实现或输出压缩类能力，可用于减少重复记录和控制产出长度。

查询原则：

- 只查当前材料类型或产出形式相关能力。
- 找到合适能力时说明能力名称、使用原因和降级方案。
- 没有合适能力时继续使用本地 Markdown 模板流程。
- 外部工具可能接触敏感信息时，先按脱敏规则处理。
- 最小化能力不得省略必要事实、风险、隐私处理和决策点。
